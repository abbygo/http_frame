...
def print_gen_testcase_log_and_formart(file_type:Text):
    '''
    生成日志和格式化文件
    :param file_type:
    :return:
    ...
    def dec(fun):
        def wrapper(*args, **kwargs):
            logger.info(f"dump testcase to {file_type} format.")
            file_name = fun(*args, **kwargs)
            logger.info(f"Generate {file_type} testcase successfully: {file_name}")
            # 只格式化py文件
...
@print_gen_testcase_log_and_formart('.PY')
...
def call_gen_python_testcase_by_yaml_or_json(open_file_name):
    '''
    调用生成测试用例方法
